...
// 修改后的分块旋转混淆算法
function encryptRotate(img1, key, blockSize = 32, iterations = 3) {
    var cv = document.createElement("canvas");
    var cvd = cv.getContext("2d");
    var wid = img1.width;
    var hit = img1.height;
    ...
    var imgdata = cvd.getImageData(0, 0, wid, hit);
    var currentData = new Uint8ClampedArray(imgdata.data);
    ...
    for (let iter = 0; iter < iterations; iter++) {
        var newData = new Uint8ClampedArray(currentData);

        // 为每个块生成旋转角度（0°, 90°, 180°, 270°）
        for (let by = 0; by < blocksY; by++) {
            for (let bx = 0; bx < blocksX; bx++) {
                var hash = md5(key + bx + "," + by + "," + iter);
                ...
                for (let y = 0; y < blockSize; y++) {
                    for (let x = 0; x < blockSize; x++) {
                        ...
                        // 复制像素数据
                        newData[newIndex] = currentData[origIndex];
                        newData[newIndex + 1] = currentData[origIndex + 1];
                        newData[newIndex + 2] = currentData[origIndex + 2];
                        newData[newIndex + 3] = currentData[origIndex + 3];
                    }
                }
            }
        }
    ...
    var oimgdata = new ImageData(currentData, wid, hit);
    cvd.putImageData(oimgdata, 0, 0);
    return [cv.toDataURL(), wid, hit];
}

function decryptRotate(img1, key, blockSize = 32, iterations = 3) {
    var cv = document.createElement("canvas");
    var cvd = cv.getContext("2d");
    var wid = img1.width;
    var hit = img1.height;
